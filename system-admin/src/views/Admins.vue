      ...
      <navbar-component></navbar-component>
        <div class="w-full overflow-x-hidden border-t flex flex-col">
            <main class="w-full flex-grow p-6">
                <h1 class="text-3xl text-black pb-6">System Admins</h1>
    
                <div class="w-full mt-2">
                <button class="bg-blue-500 text-white active:bg-pink-600 font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button" v-on:click="toggleModal()">New Admin</button>
                        ...
                        <table class="min-w-full bg-white">
                            ...
                            <tbody class="text-gray-700">
                              <pulse-loader v-if="loading" color="blue"></pulse-loader>
                                <tr v-else v-for="admin in admins" v-bind:key="admin.id">
                                    <td class="w-1/3 text-left py-3 px-4">{{admin.first_name}}</td>
                                    <td class="w-1/3 text-left py-3 px-4">{{admin.last_name}}</td>
                                    <td class="text-left py-3 px-4"><a class="hover:text-blue-500" :href="admin.email">{{admin.email}}</a></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
