...
<div v-for="(prediction, predictionName,index) in predictiondetail" v-bind:key="index">
  ...
  <div v-if="predictionName == propertyName">
    <h3 class="text-grey-dark text-sm font-medium font-sans leading-normal">Metastatic Probabilty: {{prediction[0][1]}}</h3>
  ...
  data(){
    return{
      form:{
          ...
          breastcancer: "metastatic"
      },
        systemprediction: this.$store.state.metastaticprediction,
        predictiondetail: this.$store.state.metastaticpredictiondetail,
        loading: false,
        showModal: false,
        formname: null
    }
...
created() {
  this.systemprediction = this.$store.state.metastaticprediction
  this.predictiondetail = this.$store.state.metastaticpredictiondetail
  console.log(this.systemprediction)
},
methods: {
